...
ManyToManyChannel.prototype = {
  ...
  putAsync: function putAsync(value, fn, onCaller) {
    ...
    var ret = this._putSync(value, helpers.fnHandler(fn));
  ...
  _putSync: function _putSync(value, handler) {
  ...
  takeAsync: function takeAsync(fn, onCaller) {
    ...
    var ret = this._takeSync(helpers.fnHandler(fn));
  ...
  _takeSync: function _takeSync(handler) {
    if (!handler.isActive()) {
      return;
    }
